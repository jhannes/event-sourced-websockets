...
public class IncidentReactor {
    ...
    private final List<IncidentSnapshotDto> incidents = new ArrayList<>(List.of(
            new IncidentSnapshotDto().setInfo(new IncidentDto().setDescription("Fire on the server")).setId(UUID.randomUUID()).setUpdatedAt(OffsetDateTime.now()),
            new IncidentSnapshotDto().setInfo(new IncidentDto().setDescription("Traffic on the server")).setId(UUID.randomUUID()).setUpdatedAt(OffsetDateTime.now())
    ));
    ...
    public void handleMessage(MessageToServerDto message) {
        switch (message) {
            case IncidentCommandDto command -> {
                ...
                broadcastMessage(new IncidentEventDto().putAll(command).setSequenceId(sequenceId++));
            }
        }
    }
    ...
    private void handleCommand(IncidentCommandDto command) {
        switch (command.getDelta()) {
            case CreateIncidentDeltaDto create -> incidents
                    .add(new IncidentSnapshotDto()
                            .setId(command.getIncidentId())
                            .setUpdatedAt(command.getClientTime())
                            .setInfo(create.getInfo()));
            case UpdateIncidentDeltaDto update -> incidents.stream()
                    .filter(o -> o.getId().equals(command.getIncidentId()))
